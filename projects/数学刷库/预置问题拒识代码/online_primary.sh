...
MODE="online"
LEVEL="primary"
# DATA_SUB_DIR="close/MultiPrimary"
# DATA_SUB_DIR="close/MultiPrimary2"
# DATA_SUB_DIR="close/SinglePrimary/20251217"
# DATA_SUB_DIR="close/primary1222"
# DATA_SUB_DIR="close/primary1223"
DATA_SUB_DIR="教辅数据第三批"   # NOTE 结果存放路径

EXEC_DIR="/mnt/pan8T/temp_djguo/数学-多模预置问题清洗"  # NOTE 执行路径
DATA_DIR="${EXEC_DIR}/data/${DATA_SUB_DIR}"
LOG_DIR="${EXEC_DIR}/logs/${DATA_SUB_DIR}"
if [[ ! -d "${LOG_DIR}" ]]; then
    mkdir -p ${LOG_DIR}
fi

STEP="extract"  # NOTE 修改执行的步骤
# STEP="preprocess"
# STEP="request"
# STEP="response"
# STEP="update"
...
if [[ "${STEP}" == "extract" ]]; then
    ...
    RENAME_PREFIX="doubao-seed-1-6-thinking-250615_xsji4_${today}_1114攻关_"
elif [[ "${STEP}" == "response" ]]; then
    INPUT_DIR="${DATA_DIR}/${STEP}/jsonl"
    SAVE_DIR="${DATA_DIR}/${STEP}/answer"
elif [[ "${STEP}" == "update" ]]; then
    # FILENAME="shitiping_primary_1028_单模in单模out_reject_396_with_preset.json"
    #FILENAME="shitiping_primary_1028_单模in单模out_good_num_4861_with_preset_4861.json"
    INPUT_DIR="${DATA_DIR}/raw"
